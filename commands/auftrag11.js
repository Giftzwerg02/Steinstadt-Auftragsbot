...
const aufträge11 = require('../aufträge/11.json')
const evil = eval

module.exports = {
    name: 'auftrag11',
    description: 'Auftrag mit der ID 11 ausführen',
    execute(message, auftragsnummer) {
        var auftragsindex11 = Math.floor(Math.random() * 24)
    
        message.delete();
        const embed = new Discord.MessageEmbed()
        .setTitle(`${evil(textConfig[auftragsnummer].title)}`)
        .setDescription(`${textConfig[auftragsnummer].text.join('')}`)
        ...
        message.channel.send(embed)
            .then(async m => {
                ...
                collector3.on('collect', () => {
                    ...
                    const embed = new Discord.MessageEmbed()
                    .setColor('#64cf00')
                    .setTitle(aufträge11[auftragsindex11].nummer)
                    .setDescription(`${aufträge11[auftragsindex11].fracht}\n
                    ${aufträge11[auftragsindex11].gewicht}\n
                    ${aufträge11[auftragsindex11].von}\n
                    ${aufträge11[auftragsindex11].nach}\n
                    ${aufträge11[auftragsindex11].transport}\n
                    ${aufträge11[auftragsindex11].maxSpd}\n
                    ${aufträge11[auftragsindex11].money}\n`)
                    ...
                    collector1.on('collect', () => {
                        ...
                        request.send(`{ "cash": 0, "bank": ${aufträge11[auftragsindex11].cash} }`);
                        console.log(`{ "cash": 0, "bank": ${aufträge11[auftragsindex11].cash} }`);
                        ...
                        request.onloadend = function() {
                            ...
                            `${JSON.stringify(aufträge11[auftragsindex11], null, 4)}` + 
